 ...
 * 統合 HTTP サーバー。
 *
 * cron リロードとログ取得を Hono アプリケーションとして
 * 単一の Deno.serve() で提供する。
 *
 * Discord の操作は Claude が公式 REST API を curl で直接叩くため、
 * このサーバーは Discord 用のエンドポイントを持たない。
 * ツール承認は SDK の `canUseTool` コールバックで in-process に処理する。
 */
