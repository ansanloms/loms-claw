...
本プロジェクトは cron ジョブによる定期自動実行を含む。稼働環境の件数は内部 API `GET /cron` で確認する。2026-08-23 確認時点の目安は 10 本（リポジトリ追跡の恒久ジョブ + 稼働環境のみのジョブ）。このほか `once: true` の一回限りジョブが随時追加され、実行後に自動削除される。同時刻の並列実行が瞬間的な負荷集中にならないよう、恒久ジョブは発火時刻を分散させている（`news-*` 6 本を 10 分刻みにする等）。自動実行の頻度・規模は、この「個人の通常利用（ordinary, individual usage）」の想定を外れない範囲に保つこと。
