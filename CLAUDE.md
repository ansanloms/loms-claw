...
本プロジェクトは Claude Agent SDK (`@anthropic-ai/claude-agent-sdk` の `query()`) を使用して Discord から Claude を呼び出す。

利用にあたっては以下の規約上の点を理解しておく必要がある。
...
api/server.ts              統合 HTTP サーバー。Hono アプリ作成、サブルートマウント、共通エラーハンドラ。承認は in-process のため HTTP では扱わない。
...
Claude（Agent SDK 経由）からは Bash + curl で呼び出す。API 仕様は `.claude/skills/discord/SKILL.md` に記載。
...
2. Discord REST API・cron・ログ取得を同一ポートで提供（ツール承認は in-process のため HTTP では扱わない）
3. Claude から Bash ツール経由で `curl` を実行して API を呼び出す

Discord API は Bot 起動時に常に有効化される。
