...
cron/types.ts          CronJobDef 型定義 + loadCronJobs(): .claude/cron.json の読み込みとバリデーション。
cron/executor.ts       CronExecutor: Deno.cron によるジョブ登録・実行。askClaude() → Discord 送信。
...
`Deno.cron` による定期実行ジョブ。ワークスペースの `.claude/cron.json` にジョブ定義を配列で記述する。
ファイルが存在しなければ cron 機能は無効。
...
| フィールド  | 必須 | 説明                                             |
| ----------- | ---- | ------------------------------------------------ |
| `name`      | yes  | ジョブ名（一意）。セッションキーにも使用         |
| `schedule`  | yes  | cron 式（5フィールド、UTC）                      |
| `prompt`    | yes  | Claude に送るプロンプト                          |
| `channelId` | yes  | 結果送信先の Discord チャンネル ID               |
| `maxTurns`  | no   | `CLAUDE_MAX_TURNS` のオーバーライド              |
| `timeout`   | no   | `CLAUDE_TIMEOUT` のオーバーライド（ミリ秒）      |

### 動作の仕組み

1. Bot 起動時（`ClientReady` 後）に `.claude/cron.json` を読み込み
2. 各ジョブを `Deno.cron()` で登録
3. スケジュールに従い `askClaude()` を実行
4. 結果を指定チャンネルに送信、セッション ID を `cron:{name}` キーで保存
