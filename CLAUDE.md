...
mcp/server.ts          Discord MCP HTTP サーバー。WebStandardStreamableHTTPServerTransport + Deno.serve()。
mcp/tools.ts           MCP ツール定義・登録。discord.js Client を通じて Discord API を操作。
mcp/types.ts           MCP 共通型（McpContext）。
...
Bot プロセス内で MCP HTTP サーバー（Streamable HTTP transport, stateless モード）を起動する。
Claude Code CLI (`claude -p`) が `.mcp.json` を読み込み、Discord 操作ツールを利用可能にする。

### 公開ツール

| ツール名                    | 説明                                           |
| --------------------------- | ---------------------------------------------- |
| `discord_send_message`      | 指定チャンネルにメッセージ送信                 |
| `discord_get_channel_info`  | チャンネル情報取得                             |
| `discord_list_channels`     | ギルド内チャンネル一覧                         |
| `discord_add_reaction`      | メッセージにリアクション追加                   |
| `discord_get_message`       | 指定メッセージ取得                             |
| `discord_search_messages`   | チャンネル内メッセージ検索（fetch + フィルタ） |
| `discord_get_guild_members` | ギルドメンバー一覧/検索                        |

### 前提条件

- Discord Developer Portal で **Server Members Intent**（Privileged Gateway Intent）を有効にすること。
  `discord_get_guild_members` ツールに必要。

### .mcp.json の設定

ワークスペース（Docker: `/workspace`）に `.mcp.json` を配置する。
`claude -p` が自動で読み込み、MCP サーバーに接続する。
...
ポートは `DISCORD_MCP_PORT` 環境変数（デフォルト: 3001）に合わせること。
`ENABLE_DISCORD_MCP=false` で MCP サーバーを無効化できる（デフォルト: true）。

### 動作の仕組み

1. Bot 起動時に `startMcpServer()` が `127.0.0.1:{DISCORD_MCP_PORT}` で HTTP サーバーを起動
2. `claude -p` がワークスペースの `.mcp.json` を自動読み込みし、MCP サーバーに接続
3. ツール呼び出しは PreToolUse フックによる承認を経由する
