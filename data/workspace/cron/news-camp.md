...
- テーマ: キャンプ関連の最新ニュース・キャンプ場情報・ギアレビュー・イベント等
- 優先ソース:
...
```bash
# 1 記事目: /tmp/news-camp-1.txt に 1 記事分の整形済み本文を書いてから POST
jq -Rs '{content: .}' /tmp/news-camp-1.txt | curl -sS -X POST \
  "https://discord.com/api/v10/channels/1479719413396541450/messages" \
  -H "Authorization: Bot ${DISCORD_BOT_TOKEN}" \
  -H 'Content-Type: application/json' -d @-
# 2 記事目は /tmp/news-camp-2.txt、3 記事目は /tmp/news-camp-3.txt に書いて同じ curl を繰り返す
```
