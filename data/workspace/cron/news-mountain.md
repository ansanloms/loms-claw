...
- テーマ: 登山・山岳関連の最新ニュース・山行情報・規制情報・遭難情報等
- 優先ソース:
...
```bash
# 1 記事目: /tmp/news-mountain-1.txt に 1 記事分の整形済み本文を書いてから POST
jq -Rs '{content: .}' /tmp/news-mountain-1.txt | curl -sS -X POST \
  "https://discord.com/api/v10/channels/1479719413396541450/messages" \
  -H "Authorization: Bot ${DISCORD_BOT_TOKEN}" \
  -H 'Content-Type: application/json' -d @-
# 2 記事目は /tmp/news-mountain-2.txt、3 記事目は /tmp/news-mountain-3.txt に書いて同じ curl を繰り返す
```
