...
- テーマ: 天文学・宇宙開発・地学・物理学・生物学など自然科学全般のニュース
- 優先ソース:
  - sorae.info（宇宙・天文）
...
```bash
# 1 記事目: /tmp/news-science-1.txt に 1 記事分の整形済み本文を書いてから POST
jq -Rs '{content: .}' /tmp/news-science-1.txt | curl -sS -X POST \
  "https://discord.com/api/v10/channels/1479719413396541450/messages" \
  -H "Authorization: Bot ${DISCORD_BOT_TOKEN}" \
  -H 'Content-Type: application/json' -d @-
# 2 記事目は /tmp/news-science-2.txt、3 記事目は /tmp/news-science-3.txt に書いて同じ curl を繰り返す
```
