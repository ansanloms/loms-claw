...
news-digest skill を使い、以下の引数で記事を集めろ。収集・要約のルール（優先ソースに固執しない・公開日の確認・個別 URL 必須・捏造禁止・要約 300 文字程度、RSS があればフィードを先に取得）は skill 側に従え。

- テーマ: ソフトウェア・Web 技術・OSS 関連のニュース
- 優先ソース:
  - Hacker News(news.ycombinator.com)
  - www.publickey1.jp
  - zenn.dev トレンド
  - Deno Weekly(https://uki00a.github.io/deno-weekly/index.xml)
  - Hugging News(https://huggingnews.com/)
- 件数: 3
- 期間: 直近 5 日
...
```bash
# 1 記事目: /tmp/news-tech-1.txt に 1 記事分の整形済み本文を書いてから POST
jq -Rs '{content: .}' /tmp/news-tech-1.txt | curl -sS -X POST \
  "https://discord.com/api/v10/channels/1479719413396541450/messages" \
  -H "Authorization: Bot ${DISCORD_BOT_TOKEN}" \
  -H 'Content-Type: application/json' -d @-
# 2 記事目は /tmp/news-tech-2.txt、3 記事目は /tmp/news-tech-3.txt に書いて同じ curl を繰り返す
```
