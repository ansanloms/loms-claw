...
Discord の情報取得・操作は公式 REST API (`https://discord.com/api/v10`) を Bash + curl で直接叩く。
bot トークンは環境変数 `DISCORD_BOT_TOKEN` に入っているので `Authorization: Bot ${DISCORD_BOT_TOKEN}` で渡す。
サーバー (ギルド) ID・チャンネル ID はシステムプロンプトの「Discord コンテキスト」の値を使う。

詳細は `discord` skill (`.claude/skills/discord/SKILL.md`) を参照。
