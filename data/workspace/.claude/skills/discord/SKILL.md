...
description: >-
  ...
  bot トークンを Authorization の Bot ヘッダで渡し、 https://discord.com/api/v10 を叩く。トークンは設定ファイル config.json の .discord.token から取る。メッセージ検索の API は bot に無いため、履歴を取得して jq でフィルタする。
...
- トークンは設定ファイル config.json の `.discord.token` から取る。パスは環境変数 `LOMS_CLAW_CONFIG` (未設定なら `./data/config.json`)。各コマンドで `DISCORD_BOT_TOKEN=$(jq -r '.discord.token' "${LOMS_CLAW_CONFIG:-./data/config.json}")` のように取得し、 `Authorization: Bot ${DISCORD_BOT_TOKEN}` ヘッダで渡す。トークンをクエリや body に直書きしない。会話への貼り付けやコミットも避ける。
- 取得した値が空 (`null` や空文字) なら config.json の `discord.token` 設定をユーザに確認する。
...
すべて `https://discord.com/api/v10` 配下を叩く。トークンは config.json から取得し、Authorization ヘッダで渡す。

```bash
# トークンを config.json から取得する
DISCORD_BOT_TOKEN=$(jq -r '.discord.token' "${LOMS_CLAW_CONFIG:-./data/config.json}")

curl -sS 'https://discord.com/api/v10/users/@me' \
  -H "Authorization: Bot ${DISCORD_BOT_TOKEN}"
```

- **Bash コマンドごとに環境変数は持ち越せない。** 以降の各例は `${DISCORD_BOT_TOKEN}` を使うが、Discord API を叩く Bash 実行の冒頭で必ず上の取得行を入れてから curl を呼ぶこと。
...
- トークンは秘密情報。config.json から取得して Authorization ヘッダで渡し、コマンドラインや body・ログに平文を残さない。
