...
- **撤退基準** (計画配下、登山・ハイキング時): カットオフライン (チェックポイント×時刻の撤退基準) を決行宣言までに確定させる。前提 (起点・ルート) が未定なら案として記録する。安全判断に直結するため、登山を含む旅行では省略しない。
- **一時 cron 記録** (計画配下): この旅行向けの一時 cron を仕込んだら、ジョブ名・発火時刻・対象日・プロンプト要点を 1 行で追記する。削除・再設定・障害と補填も記録する。`once` ジョブは失敗しても定義ファイルが消えるため、この記録が補填内容を再現する唯一の手がかりになる。
...
- 全体像は `memory/travel/index.md` を見る (status 別一覧 + 直近の旅行サマリ。cron ジョブ travel-digest が週次で自動生成)。index.md は自動生成物なので手で編集しない。ノートを更新しても index.md への反映は不要 (週次 cron が追従する)。初回生成前は存在せず、それ自体は異常ではない。存在しない場合は次の `ls memory/travel/` の手順で代替する。
