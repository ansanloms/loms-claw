...
Bot プロセスはメモリ上に直近のログをリングバッファで保持している。内部 API 経由で取得できる。
...
次のものが実在する名前空間の**全て**。ここに無い名前空間を `namespace` に指定してもサーバーはエラーを返さず、単に該当ログが無いだけの空配列 `[]` が 200 で返る。**綴りを間違えると「そのログは存在しない」と誤読するので、必ずこの一覧から選ぶこと。** 廃止済みの内部 Discord API ハンドラ用だった名前空間はこの一覧に存在しない（指定しても常に空配列になる）。
...
bot プロセスが停止している、またはポート番号が違うと curl 自体が失敗する（`Connection refused` 等）。ポートは `claude.apiPort` で変わるので、`config.json` を確認するか、まず他のエンドポイント（例: `/cron`）が叩けるか確かめる。
